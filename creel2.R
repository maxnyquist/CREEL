################################### HEADER ###################################
#   TITLE: Lake Trout Tagging Length Weight Relationships
#   DESCRIPTION: This script will be used to combine the excel spreadsheets sent by MA DFW containing fish ID and sample ID data. 
#   It will also be used to calculate the length weight relationship formula for Wachusett Lake Trout (LT) and Smallmouth Bass (SMB). 
#   AUTHOR(S): Max W. Nyquist
#   DATE LAST UPDATED: 5/29/2019
#   R VERSION 32 bit R Portable
##############################################################################.


### LOAD PACKAGES ####
pkg <- c("tidyverse", "magrittr", "lubridate","RODBC", "DBI", "odbc", "colorspace", "readxl", "dplyr")
...
fish <- dbReadTable(con, "Fish Caught Query")
#Table for Estimating Totals Excel Workbook
filename <- "W:/WatershedJAH/EQStaff/Aquatic Biology/Fish/Creel Survey related/2017 Creel Survey/2017 Table for Estimating Totals.xlsx"
...
totals <- read_excel_allsheets(filename)
#Query becomes dataframe
